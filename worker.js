...
// ===== 웩슬러(WAIS) 전용 사전 =====
// VCI: 격식/고급 어휘 (한자어, 격식체)
const FORMAL_WORDS = ['따라서','그러므로','더불어','뿐만아니라','비록','그럼에도','한편','결론적으로','요컨대','즉','예컨대','다시말해','반드시','궁극적으로','본질적으로','근본적으로','원칙적으로','일반적으로','기본적으로'];
const ADVANCED_VOCAB = ['함의','맥락','관점','측면','양상','현상','개념','원리','구조','체계','범주','기준','원칙','논거','전제','가설','귀결','인과','상관','본질','근거','타당','논리','합리','이성','판단','해석','추론','유추','비유'];

// FRI: 조건부 추론 / 문제해결
const CONDITIONAL_WORDS = ['만약','가정하면','그렇다면','라면','경우에','전제로','조건이','상황에서','한다면','된다면','였다면','었더라면','이었으면'];
const PROBLEM_SOLVE = ['해결','방법','대안','시도','접근','전략','방안','대처','해법','개선','보완','수정','대응','조치','해결책','어떻게하면'];

// WMI: 맥락 참조 (이전 대화 참조)
const CONTEXT_REF = ['아까','그때','전에','방금','좀전에','이전에','위에서','말했듯이','했잖아','그랬잖아','말한것처럼','아까그','그때그','전에말한','다시말하면'];
...
function extractStats(messages, partnerFromHeader) {
  ...
  participants.forEach(name => {
    ...
    // 웩슬러 전용 카운터
    let mFormalWord = 0, mAdvancedVocab = 0;
    let mConditional = 0, mProblemSolve = 0;
    let mContextRef = 0;
    let mMultiClause = 0;
    let mLongWords = 0;
    const msgLengths = [];
    ...
    memberMsgs.forEach(msg => {
      ...
      // ===== 웩슬러 전용 신호 =====
      FORMAL_WORDS.forEach(w => { if (t.includes(w)) mFormalWord++; });
      ADVANCED_VOCAB.forEach(w => { if (t.includes(w)) mAdvancedVocab++; });
      CONDITIONAL_WORDS.forEach(w => { if (t.includes(w)) mConditional++; });
      PROBLEM_SOLVE.forEach(w => { if (t.includes(w)) mProblemSolve++; });
      CONTEXT_REF.forEach(w => { if (t.includes(w)) mContextRef++; });
      // 다중절 문장 (접속사/연결어 2개 이상 = 높은 작업기억)
      const clauseHits = t.match(RE_COMPLEXITY);
      if (clauseHits && clauseHits.length >= 2) mMultiClause++;
      // 긴 단어 (4자 이상 = 어휘 깊이)
      const cleanedForLong = t.replace(RE_EMOJI, '').replace(/[ㅋㅎㅠㅜ]+/g, '');
      const longW = cleanedForLong.split(/\s+/).filter(w => w.length >= 4);
      mLongWords += longW.length;
    });
    ...
    perMemberStats.push({
      ...
      // 웩슬러 전용 비율
      formalWordRatio: textCount > 0 ? mFormalWord / textCount : 0,
      advancedVocabRatio: textCount > 0 ? mAdvancedVocab / textCount : 0,
      conditionalRatio: textCount > 0 ? mConditional / textCount : 0,
      problemSolveRatio: textCount > 0 ? mProblemSolve / textCount : 0,
      contextRefRatio: textCount > 0 ? mContextRef / textCount : 0,
      multiClauseRatio: textCount > 0 ? mMultiClause / textCount : 0,
      longWordRatio: mTotalWords > 0 ? mLongWords / mTotalWords : 0,
    });
  });
...
function computeWechsler(memberStats) {
  ...
  return memberStats.map(ms => {
    ...
    // WAIS: 어휘력, 언어 개념 형성, 단어 지식, 언어 추론
    // 신호: 어휘 다양성, 고급어/격식체, 긴 단어, 문장 복잡도, 비속어(-), 추상어
    raw.vci = clamp(0, 100,
      S(ms.vocabDiversity, avg('vocabDiversity'), 12) +
      S(ms.formalWordRatio, avg('formalWordRatio'), 10) +
      S(ms.advancedVocabRatio, avg('advancedVocabRatio'), 8) +
      S(ms.longWordRatio, avg('longWordRatio'), 8) +
      S(ms.complexityPerMsg, avg('complexityPerMsg'), 6) +
      S_inv(ms.profanityRatio, avg('profanityRatio'), 6) +
      S(ms.abstractImpressionRatio, avg('abstractImpressionRatio'), 6) +
      R(ms.vocabDiversity, avg('vocabDiversity'), std('vocabDiversity'), 10) +
      R(ms.formalWordRatio, avg('formalWordRatio'), std('formalWordRatio'), 8) +
      R(ms.advancedVocabRatio, avg('advancedVocabRatio'), std('advancedVocabRatio'), 8) +
      R(ms.longWordRatio, avg('longWordRatio'), std('longWordRatio'), 6) +
      R(ms.complexityPerMsg, avg('complexityPerMsg'), std('complexityPerMsg'), 6) +
      R(ms.abstractImpressionRatio, avg('abstractImpressionRatio'), std('abstractImpressionRatio'), 6)
    );

    // ===== FRI: 유동추론 (Fluid Reasoning) =====
    // WAIS: 패턴 인식, 추상적 추론, 귀납/연역, 양적 추론
    // 신호: 인과표현, 조건추론, 문제해결, 분석표현, 수량추론, 분류/비교
    raw.fri = clamp(0, 100,
      S(ms.causalRatio, avg('causalRatio'), 10) +
      S(ms.conditionalRatio, avg('conditionalRatio'), 10) +
      S(ms.problemSolveRatio, avg('problemSolveRatio'), 8) +
      S(ms.analyticalExprRatio, avg('analyticalExprRatio'), 8) +
      S(nuPerMsg, nuPerMsgAvg, 8) +
      S(ms.classifyRatio, avg('classifyRatio'), 6) +
      S(ms.linkRatio, avg('linkRatio'), 6) +
      R(ms.causalRatio, avg('causalRatio'), std('causalRatio'), 8) +
      R(ms.conditionalRatio, avg('conditionalRatio'), std('conditionalRatio'), 8) +
      R(ms.problemSolveRatio, avg('problemSolveRatio'), std('problemSolveRatio'), 6) +
      R(nuPerMsg, nuPerMsgAvg, nuPerMsgStd, 6) +
      R(ms.analyticalQuestionRatio, avg('analyticalQuestionRatio'), std('analyticalQuestionRatio'), 6) +
      R(ms.hypotheticalQuestionRatio, avg('hypotheticalQuestionRatio'), std('hypotheticalQuestionRatio'), 6) +
      R(ms.classifyRatio, avg('classifyRatio'), std('classifyRatio'), 4)
    );

    // ===== WMI: 작업기억 (Working Memory) =====
    // WAIS: 정보 유지·조작, 순서 기억, 주의력, 정신적 조작
    // 신호: 맥락 참조(아까/그때), 다중절 문장, 문장 복잡도, 긴 문장, 주제 유지, 연결어
    raw.wmi = clamp(0, 100,
      S(ms.contextRefRatio, avg('contextRefRatio'), 14) +
      S(ms.multiClauseRatio, avg('multiClauseRatio'), 12) +
      S(ms.complexityPerMsg, avg('complexityPerMsg'), 10) +
      S(ms.avgSentenceLength, avg('avgSentenceLength'), 8) +
      S(ms.topicDriverRatio, avg('topicDriverRatio'), 6) +
      S(ms.connectiveRatio, avg('connectiveRatio'), 6) +
      R(ms.contextRefRatio, avg('contextRefRatio'), std('contextRefRatio'), 12) +
      R(ms.multiClauseRatio, avg('multiClauseRatio'), std('multiClauseRatio'), 8) +
      R(ms.complexityPerMsg, avg('complexityPerMsg'), std('complexityPerMsg'), 8) +
      R(ms.avgSentenceLength, avg('avgSentenceLength'), std('avgSentenceLength'), 6) +
      R(ms.topicDriverRatio, avg('topicDriverRatio'), std('topicDriverRatio'), 5) +
